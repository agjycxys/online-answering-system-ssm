			...
			function reset(){
				<c:forEach items="${usersZGT }" var="usersZGT">
					if("${usersZGT.ispingyue}"=="0"){
						alert("该考生试卷处于没人评阅状态,您不能更改!");
					}
					if("${usersZGT.ispingyue}"!="0"){
						window.location.href = "${pageContext.request.contextPath}/ResetServlet?openid=${usersZGT.openid }";
					}
				</c:forEach>
			}
		</script>
		...
		<c:forEach items="${usersZGT }" var="usersZGT" varStatus="status">
			<table class="table table-striped table-bordered">
				...
				<tr>
					<td>更改评阅状态</td>
					<td>
						<a href="javascript:void(0)" class="layui-btn layui-btn-xs" onclick="reset()">更改</a>
					</td>
				</tr>
					</table>
			</c:forEach>
